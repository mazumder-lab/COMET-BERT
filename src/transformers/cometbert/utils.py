...
def use_experts(
    layer_idx, perm=False
):  # This function can be modified to use usual BertLayer instead of CometBertLayer
    return True
...
class ImportanceProcessor:
    def __init__(self, config, layer_idx, num_local_experts, local_group_rank):
        self.num_experts = config.cometbert_expert_num  # total number of experts
        ...
        self.intermediate_size = config.cometbert_expert_dim  # FFN hidden dimension
        self.share_importance = config.cometbert_share_importance  # number of shared FFN dimension

        importance = ImportanceProcessor.load_importance_single(config.cometbert_load_importance)[layer_idx, :]
