...
from .cometbert.utils import process_ffn
...
class Trainer:
    ...
    def __init__(
        ...
        default_label_names = (
            ["start_positions", "end_positions"] if type(self.model).__name__ in MODEL_FOR_QUESTION_ANSWERING_MAPPING_NAMES.values() else ["labels"]
        )
    ...
    def _get_train_sampler(self) -> Optional[torch.utils.data.sampler.Sampler]:
        if isinstance(self.train_dataset, torch.utils.data.IterableDataset) or not isinstance(self.train_dataset, collections.abc.Sized):
            return None
        ...
        if self.args.group_by_length:
            ...
            if self.args.world_size <= 1:
                return LengthGroupedSampler(self.train_dataset, self.args.train_batch_size, model_input_name=model_input_name)
            else:
            ...
            if self.args.world_size <= 1:
                ...
                return DistributedSampler(self.train_dataset, num_replicas=self.args.world_size, rank=self.args.process_index)
    ...
    def create_optimizer(self) -> torch.optim.Optimizer:
        ...
        optimizer_grouped_parameters = [
            {"params": [p for n, p in self.model.named_parameters() if n in decay_parameters], "weight_decay": self.args.weight_decay},
            {"params": [p for n, p in self.model.named_parameters() if n not in decay_parameters], "weight_decay": 0.0},
        ]
        ...
        if self.sharded_ddp == ShardedDDPOption.SIMPLE:
            return OSS(params=optimizer_grouped_parameters, optim=optimizer_cls, **optimizer_kwargs)
    ...
    def create_scheduler(self, optimizer: torch.optim.Optimizer, num_training_steps: int) -> torch.optim.lr_scheduler.LambdaLR:
        ...
        return get_scheduler(self.args.lr_scheduler_type, optimizer, num_warmup_steps=warmup_steps, num_training_steps=num_training_steps)
    ...
    def _wrap_model(self, model, training=True):
        ...
        if self.sharded_ddp is not None:
            ...
            model = torch.nn.parallel.DistributedDataParallel(
                model, device_ids=[self.args.local_rank], output_device=self.args.local_rank, find_unused_parameters=find_unused_parameters
            )
    ...
    def train(self, resume_from_checkpoint: Optional[Union[str, bool]] = None, trial: Union["optuna.Trial", Dict[str, Any]] = None, **kwargs):
        ...
        if "model_path" in kwargs:
            ...
            warnings.warn(
                "`model_path` is deprecated and will be removed in a future version. Use `resume_from_checkpoint` " "instead.", FutureWarning
            )
        if len(kwargs) > 0:
        ...
        require_process = self.model.config.cometbert_load_experts
        if require_process:
        ...
        logger.info(
            "num. model params: {:,} (num. trained: {:,})".format(
                sum(p.numel() for p in self.model.parameters()), sum(p.numel() for p in self.model.parameters() if p.requires_grad)
            )
        )
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            steps_in_epoch = len(epoch_iterator) if train_dataset_is_sized else self.args.max_steps * self.args.gradient_accumulation_steps
    ...
    def _load_optimizer_and_scheduler(self, checkpoint):
        ...
        if os.path.isfile(os.path.join(checkpoint, "optimizer.pt")) and os.path.isfile(os.path.join(checkpoint, "scheduler.pt")):
            ...
            if is_torch_tpu_available():
                ...
                self.optimizer.load_state_dict(torch.load(os.path.join(checkpoint, "optimizer.pt"), map_location=self.args.device))
                with warnings.catch_warnings(record=True) as caught_warnings:
    ...
    def save_model(self, output_dir: Optional[str] = None):
        ...
        elif ShardedDDPOption.ZERO_DP_2 in self.args.sharded_ddp or ShardedDDPOption.ZERO_DP_3 in self.args.sharded_ddp:
        ...
        if not isinstance(self.model, PreTrainedModel):
            if isinstance(unwrap_model(self.model), PreTrainedModel):
                unwrap_model(self.model).save_pretrained(
                    output_dir, save_config=self.is_world_process_zero(), state_dict=self.model.state_dict(), save_function=xm.save
                )
            else:
    ...
    def predict(self, test_dataset: Dataset, ignore_keys: Optional[List[str]] = None, metric_key_prefix: str = "eval") -> PredictionOutput:
        """
        ...
        output = self.prediction_loop(test_dataloader, description="Prediction", ignore_keys=ignore_keys, metric_key_prefix=metric_key_prefix)
